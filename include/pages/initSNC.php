...
try {
    ...
    fwrite($file, PHP_EOL.$Now->format('Y-m-d H:i:s')." ---> initSNC.php". PHP_EOL);
    ...
    if(($_POST['id']>=0)){
        ...
        $text = "SELECT * FROM snc WHERE clientID = '".$_POST['id']."'";
    }else{
        fwrite($file, "SESSION[userid]=".$_SESSION['userid'].PHP_EOL);
        $text = "SELECT * FROM snc WHERE clientID = '".$_SESSION['userid']."'";
    }
